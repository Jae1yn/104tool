 ...
 * 工具配置。
 *
 * @param connectionMode           连接建立方式：DIAL（工具拨号连接子站，默认）或 LISTEN（工具监听，子站拨入）
...
public record Settings(ConnectionMode connectionMode, String substationHost, int port, int commonAddress,
    ...
    /** 旧版 settings.json 缺失新字段时，Jackson 会传入 null。回落到 LISTEN（老配置生成时只有监听模式，升级不改变既有行为）。 */
    public Settings {
    ...
    public static Settings defaults() {
        return new Settings(ConnectionMode.DIAL, "127.0.0.1", 2404, 1, 5000, false, false);
    }
