...
/**
 * 模拟子站：连接主站替身，应答总召/对时/遥控，周期性上送遥测遥信。
 * 用于端到端验证和演示。用法: SimulatedSubstation [host] [port] [运行秒数]
 */
public final class SimulatedSubstation {

    private static final int COMMON_ADDRESS = 1;

    public static void main(String[] args) throws Exception {
        String host = args.length > 0 ? args[0] : "127.0.0.1";
        int port = args.length > 1 ? Integer.parseInt(args[1]) : 2404;
        long runSeconds = args.length > 2 ? Long.parseLong(args[2]) : 300;

        Connection connection = new ClientConnectionBuilder(host)
                .setPort(port)
                .setConnectionEventListener(new Responder())
                .build();
        connection.startDataTransfer();
        System.out.println("已连接主站替身 " + host + ":" + port + "，STARTDT 完成");

        long deadline = System.currentTimeMillis() + runSeconds * 1000;
        int tick = 0;
        while (System.currentTimeMillis() < deadline && !connection.isClosed()) {
            Thread.sleep(2000);
            tick++;
            boolean on = tick % 2 == 0;
            ...
            try {
                ...
                connection.send(new ASdu(ASduType.M_SP_TB_1, false, CauseOfTransmission.SPONTANEOUS,
                        false, false, 0, COMMON_ADDRESS,
                        new InformationObject(1002,
                                new IeSinglePointWithQuality(!on, false, false, false, false),
                ...
                System.out.println("上送失败，连接已断开: " + e.getMessage());
                break;
            }
        }
        connection.close();
        System.out.println("模拟子站退出");
    }
    ...
    private static final class Responder implements ConnectionEventListener {
        ...
        public void connectionClosed(Connection connection, IOException cause) {
            System.out.println("连接关闭: " + (cause == null ? "" : cause.getMessage()));
        }
